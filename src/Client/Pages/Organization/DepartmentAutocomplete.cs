...
public class DepartmentAutocomplete : MudAutocomplete<Guid>
{
    ...
    private List<DepartmentDto> _entityList = new();
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender &&
            ...
            await ApiHelper.ExecuteCallGuardedAsync(
                () => DepartmentsClient.GetAsync(_value), Snackbar) is { } entityDetailsDto)
        {
    ...
    private async Task<IEnumerable<Guid>> SearchDepartments(string value)
    {
        ...
        {
            BusinessUnitId = FatherId == Guid.Empty ? null : FatherId,
            PageSize = 10,
